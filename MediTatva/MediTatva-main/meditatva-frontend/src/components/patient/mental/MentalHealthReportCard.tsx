        ...
        <CardContent className="space-y-6 max-h-[76vh] overflow-y-auto pr-1 sm:pr-2">
          {responseRows.length > 0 && (
            <div className="rounded-lg bg-white dark:bg-slate-800/50 p-4 border border-slate-200 dark:border-slate-700">
              <h4 className="font-semibold text-slate-900 dark:text-white mb-3">Submitted Survey Responses</h4>
              <div className="space-y-2">
                {responseRows.map((row, index) => (
                  <div
                    key={`${row.questionId}-${index}`}
                    className="rounded-md bg-slate-50 dark:bg-slate-900/40 p-3 border border-slate-200 dark:border-slate-700"
                  >
                    <p className="text-sm font-medium text-slate-900 dark:text-white">{row.questionText}</p>
                    <p className="text-sm text-slate-700 dark:text-slate-300 mt-1">
                      {row.answerLabel} (Score: {row.score})
                    </p>
                  </div>
                ))}
              </div>
